...
```cmake
cmake_minimum_required(VERSION 3.10) # cmake设置要求的最低版本
project(ProjectName)                 # 设置项目名称
project(ProjectName VERSION 1.0)     # 配置项目版本号

set(CMAKE_CXX_STANDARD 11)            # 编译目标时需要支持的C++特性
set(CMAKE_CXX_STANDARD_REQUIRED True) # 要求编译器支持指定的C++特性
